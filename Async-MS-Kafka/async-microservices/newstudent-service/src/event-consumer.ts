...
const eventListner = async () => {
  ...
  await consumer.run({
    ...
    eachMessage: async ({topic, partition, message}) => {
      ...
      if (newMessage.type === 'checkeligibility' || newMessage.type === 'documentation') {
        switch (newMessage.type) {
          case 'checkeligibility':
            workflow.history.checkeligibility = newMessage.result;
            break;
          case 'documentation':
            workflow.history.documentation = newMessage.result;
            break;
          default:
            break;
        }
